...
- `scripts/sync_trainingpeaks_plan.py` publishes the TrainingPeaks coach plan for the Train tab and its minimal grey calendar labels. It retains historic activities, while refreshing the previous three days and up to 56 days ahead. The scheduled publisher refreshes the JSON daily and only commits when it has changed.
